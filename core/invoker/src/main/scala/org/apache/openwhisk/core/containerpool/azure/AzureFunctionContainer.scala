...
import org.apache.openwhisk.core.azure.{AzureFunctionAction, AzureFunctionStore}
...
case class AzureFunctionContainer(action: AzureFunctionAction, store: AzureFunctionStore)(
  implicit override protected val as: ActorSystem,
  protected val ec: ExecutionContext,
  protected val logging: Logging)
    extends Container {
  override protected val id: ContainerId = ContainerId(action.name)
  ...
  override protected def callContainer(path: String,
    ...
    //TODO Use timeout
    require(!retry, "Retry should be false as AzureFunctionContainer only supports '/run' call")
    logging.info(this, s"Invoking Azure Function $action")
    store.invokeFunction(action, body)
  }
